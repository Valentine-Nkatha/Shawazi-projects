...
const TransactionsPage: React.FC = () => {
  ...
  <div className="images-container mb-6 mx-2 sm:mx-4 md:mx-6 lg:mx-8 xl:mx-12">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
      {transactions
        .filter(transaction => transaction.sellerUploaded && transaction.sellerImageUrl)
        .map((transaction, idx) => (
          <div key={idx} className="border rounded-lg p-2">
            <Image
              src={transaction.sellerImageUrl || '/path/to/fallback/image.jpg'} // Ensure the src is a valid string
              alt={`Uploaded image for transaction ${idx + 1}`}
              className="w-full h-auto rounded-md"
              width={500} // Set width
              ...
              <table className="min-w-full text-left text-sm">
                ...
                <tbody>
                  {transactions.length > 0 ? (
                    transactions
                      .filter(transaction => 
                        userType === "buyer" ? transaction.buyerUploaded : transaction.sellerUploaded
                      )
