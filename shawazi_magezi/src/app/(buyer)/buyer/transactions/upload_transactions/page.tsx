...
const TransactionsPage: React.FC = () => {
  ...
  useEffect(() => {
    const storedUserType = Cookies.get("userRole");
    setUserType(storedUserType || "buyer");

    const loadData = async () => {
      try {
        const [transactionsData, agreementsData] = await Promise.all([
          fetchTransaction(),
          fetch("/api/agreements").then(res => {
    ...
    if (agreementId) {
      ...
      if (agreement) {
        ...
        const completedTransactions = transactions
          .filter(t => t.agreement_id === agreementId && t.status === "Complete")
          .reduce((sum, t) => sum + t.amount, 0);
        setRemainingAmount(agreement.remaining_amount - completedTransactions);
      } else {
        setSelectedAgreement(null);
        setRemainingAmount(0);
      }
    }
  }, [agreementId, agreements, transactions]);

  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!image || !agreementId) {
      setMessage("Please select both an image and an agreement.");
    ...
    try {
      ...
      setIsModalOpen(true);
      setImage(null);
      const updatedTransactions = await fetchTransaction();
  ...
  const getUploadStatus = (transaction: Transaction) => {
    const statuses = [];
    if (transaction.buyerUploaded) statuses.push("Buyer ✓");
    if (transaction.sellerUploaded) statuses.push("Seller ✓");
    return statuses.join(" | ") || "Pending Uploads";
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col lg:flex-row">
      <div className="lg:w-64 xl:w-72 flex-shrink-0 lg:sticky lg:top-0 lg:h-screen">
        <BuyerSidebar />
      </div>
      
      <div className="flex-1 p-4 sm:p-6 md:p-6 lg:p-8 overflow-x-hidden">
        <div className="relative flex items-center justify-center mb-6">
          <Link href="/dashboard" className="absolute left-0 top-1/2 transform -translate-y-1/2">
            <IoArrowBackOutline className="text-[28px] hover:bg-secondary" />
          </Link>
          <h1 className="text-3xl font-bold text-primary">Transactions</h1>
        </div>

        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <h2 className="text-xl font-semibold mb-4">Upload Transaction Receipt</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="border-2 border-dashed border-gray-300 p-4 text-center">
              <MdFileUpload className="mx-auto text-4xl" />
              <input
                type="file"
                accept="image/*"
                onChange={(e) => setImage(e.target.files?.[0] || null)}
            ...
            <div>
              <label className="block text-sm font-medium mb-1">Select Agreement</label>
              <select
                value={agreementId}
                onChange={(e) => setAgreementId(e.target.value)}
              ...
              >
                ...
                {agreements.map((agreement) => (
                  <option key={agreement.agreement_id} value={agreement.agreement_id}>
                    {agreement.parcel_number} - Remaining: KSH {agreement.remaining_amount}
                  </option>
                ))}
              </select>
            </div>
            ...
            {selectedAgreement && (
              <div className="bg-gray-50 p-4 rounded">
                ...
                <p>Parcel Number: {selectedAgreement.parcel_number}</p>
                <p>Remaining Amount: KSH {remainingAmount}</p>
                <p>Status: {selectedAgreement.buyer_agreed && selectedAgreement.seller_agreed ? 'Both Parties Agreed' : 'Pending Agreement'}</p>
              </div>
            )}

            <button
              type="submit"
              className="w-full bg-hover text-white py-2 rounded hover:bg-secondary"
            >
              Submit Transaction
            </button>
          </form>
            ...
            <table className="min-w-full">
              <thead className="bg-hover text-white">
                <tr>
                  ...
                  <th className="px-4 py-2">Agreement ID</th>
              ...
              <tbody>
                {transactions.map((transaction, idx) => (
                  <tr key={idx} className="border-b">
                    ...
                    <td className="px-4 py-2">{transaction.agreement_id}</td>
                    ...
                    <td className="px-4 py-2">{getUploadStatus(transaction)}</td>
                    <td className="px-4 py-2">{transaction.unique_code}</td>
                  </tr>
                ))}
              </tbody>
            </table>
