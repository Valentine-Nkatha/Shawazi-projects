...
const TransactionsPage: React.FC = () => {
  ...
  const [agreementId, setAgreementId] = useState<string>("");
  const [message, setMessage] = useState<string>("");
  const [userType, setUserType] = useState<string>(() => Cookies.get("userType") || "buyer");
  const [isModalOpen, setIsModalOpen] = useState<boolean>(false);
  const [isErrorModalOpen, setIsErrorModalOpen] = useState<boolean>(false);

  const filterTransactionsByUserType = (allTransactions: Transaction[], currentUserType: string) => {
    console.log('Filtering transactions for user type:', currentUserType);
    console.log('All transactions:', allTransactions);
    
    const filtered = allTransactions.filter(transaction => {
      // For buyer
      if (currentUserType === "buyer") {
        return transaction.buyerUploaded === true || transaction.buyerImageUrl;
      }
      // For seller
      return transaction.sellerUploaded === true || transaction.sellerImageUrl;
    });
  ...
  // Load transactions initially and when userType changes
  useEffect(() => {
    const loadTransactions = async () => {
      try {
        const data = await fetchTransaction();
        console.log("Fetched transactions:", data);
        setTransactions(data);
        filterTransactionsByUserType(data, userType);
        setIsLoading(false);
      } catch (err) {
        setError(err instanceof Error ? err.message : "Unknown error");
        setIsLoading(false);
  ...
  // Separate useEffect to handle filtering when transactions change
  useEffect(() => {
    filterTransactionsByUserType(transactions, userType);
  }, [transactions, userType]);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Refresh transactions immediately after successful upload
      const updatedData = await fetchTransaction();
      console.log("Updated transactions after post:", updatedData);
      setTransactions(updatedData);
      // Explicitly filter the new data
      filterTransactionsByUserType(updatedData, userType);
  ...
  const handleUserTypeChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    const selectedUserType = e.target.value;
    console.log("User type changed to:", selectedUserType);
    setUserType(selectedUserType);
    Cookies.set("userType", selectedUserType);
    filterTransactionsByUserType(transactions, selectedUserType);
  };
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col lg:flex-row">
      ...
      <div className="flex-1 p-4 sm:p-6 md:p-6 lg:p-8 md:pl-8 lg:pl-12 xl:pl-16 overflow-x-hidden md:max-w-[calc(100%-16rem)] lg:max-w-none">
        ...
        {/* Display uploaded images for seller */}
        {userType === "seller" && (
          <div className="images-container mb-6 mx-2 sm:mx-4 md:mx-6 lg:mx-8 xl:mx-12">
            <h2 className="text-xl sm:text-2xl mb-4">Your Uploaded Images</h2>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredTransactions.map((transaction, idx) =>
                transaction.sellerImageUrl ? (
                  <div key={idx} className="border rounded-lg p-2">
                    <Image
                      src={transaction.sellerImageUrl}
                      alt={`Uploaded image for transaction ${idx + 1}`}
