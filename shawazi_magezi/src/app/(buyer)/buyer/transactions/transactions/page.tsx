      ...
      <div className="flex-grow p-1 xs:p-2 sm:p-4 ml-16 xs:ml-20 sm:ml-[200px] md:ml-[250px] lg:ml-[300px]">
        ...
        <div className="w-full max-w-5xl mt-2 xs:mt-4 sm:mt-10 mx-auto">
          <div className="flex flex-col xs:flex-row justify-between gap-2 xs:gap-4 mb-2 sm:mb-4">
            <Link href="/transactions/history-of-transactions" className="w-[180px] xs:w-auto">
            ...
            <Link href="/buyer/transactions/upload_transactions" className="w-[180px] xs:w-auto">
              <button className="custom-button bg-hover text-white w-[180px] xs:w-[140px] py-1.5 xs:py-2 px-2 xs:px-3 sm:px-4 text-xs xs:text-sm rounded-lg">
                Upload Payments
              </button>
            </Link>
            <Link href="/buyer/transactions/history-of-transactions" className="w-[180px] xs:w-auto">
              <button className="custom-button bg-hover text-white w-[180px] xs:w-[140px] py-1.5 xs:py-2 px-2 xs:px-3 sm:px-4 text-xs xs:text-sm rounded-lg">
                View More
              </button>
            </Link>
            ...
            <table className="w-full text-left border-collapse min-w-[300px]">
              <thead>
                <tr className="border-b">
                  <th className="p-1 xs:p-2 text-black text-xs xs:text-sm md:text-base lg:text-lg">Date</th>
                  <th className="p-1 xs:p-2 text-black text-xs xs:text-sm md:text-base lg:text-lg">Status</th>
                  <th className="p-1 xs:p-2 text-black text-xs xs:text-sm md:text-base lg:text-lg">Amount</th>
                  <th className="p-1 xs:p-2 text-black text-xs xs:text-sm md:text-base lg:text-lg">Code</th>
                </tr>
              </thead>
              <tbody>
                {isLoading ? (
                  ...
                  transactions
                    ...
                    .map((transaction, idx) => (
                      <tr key={idx} className="border-b border-primary">
                        <td className="p-1 xs:p-2 text-xs xs:text-sm md:text-base lg:text-lg">{formatDate(transaction.date)}</td>
                        ...
                        <td className="p-1 xs:p-2 text-xs xs:text-sm md:text-base lg:text-lg">{transaction.amount}</td>
                        <td className="p-1 xs:p-2 text-xs xs:text-sm md:text-base lg:text-lg">{transaction.unique_code}</td>
                      </tr>
                    ))
                ) : (
                  <tr>
